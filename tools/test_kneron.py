...
def parse_args():
    ...
    parser.add_argument(
        '--out-kneron', help='dump result file for kneron test format')
    ...
    if rank == 0:
        ...
        if args.metrics:
            ...
            for k, v in eval_results.items():
                ...
                print(f'\n{k} : {v}')
        if args.out or args.out_kneron:
            ...
            if args.out_kneron:
                import json
                output_list = list()
                for score, class_id, info in zip(results['pred_score'],
                                                 results['pred_label'],
                                                 dataset.data_infos):
                    output_list.append({
                        'img_path':
                        os.path.join(info['img_prefix'],
                                     info['img_info']['filename']),
                        'class1': [float(score), int(class_id)]
                    })
                with open(args.out_kneron, 'w') as f:
                    json.dump(output_list, f, ensure_ascii=True, indent=4)
