...
def eliminate_Identify_and_Dropout(g):
    ...
    for node in g.node:
        ...
        for following_node in following_nodes:
            modhelper.replace_node_input(following_node, node.output[0],
                                         node.input[0])
        # Delete value info
...
def remove_useless_last_nodes(g):
    ...
    """
    USELESS = [
        "Reshape", "Identity", "Transpose", "Flatten", "Dropout", "Mystery",
        "Constant", "Squeeze", "Unsqueeze", 'Softmax'
    ]
    graph = Graph(g)
    ...
    while todo:
        ...
        if cur_node.proto.op_type not in USELESS:
            continue
        # Find the output
        cur_node_output = helper.find_output_by_name(g,
                                                     cur_node.proto.output[0])
...
def eliminate_shape_changing_after_input(g):
    ...
    node_to_remove = []
    REMOVE_LIST = [
        "Reshape", "Transpose", "Flatten", "Dropout", "Squeeze", "Unsqueeze"
    ]
    for node in g.node:
        ...
        if node.op_type == 'Reshape':
            ...
            new_input = onnx.helper.make_tensor_value_info(
                ...
                output_val_info.type.tensor_type.elem_type, new_shape)
...
def eliminate_consecutive_reshape(g):
    ...
    for node in g.node:
        ...
        new_reshape_node = onnx.helper.make_node(
            'Reshape', [pre_data_node.input[0], node.input[1]],
            [node.output[0]],
            name=node.output[0])
...
def eliminate_trivial_maxpool(g):
    ...
    for node in g.node:
        ...
        for next_node in next_nodes:
            modhelper.replace_node_input(next_node, node.output[0],
                                         node.input[0])
...
def eliminate_nop_pads(g):
    ...
    for node in g.node:
        ...
        if len(node.input) > 2:
            constant_value_node = helper.find_node_by_output_name(
                g, node.input[2])
        # If this node is the output node, set its previous node as output nodes.
        if helper.find_output_by_name(g, node.output[0]) is not None:
            todel_output = helper.find_output_by_name(g, node.output[0])
